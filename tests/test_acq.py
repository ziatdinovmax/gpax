...
def test_Thompson_gp():
    X = onp.random.randn(8,)
    X_new = onp.random.randn(12,)
    y = 10 * X**2
    m = ExactGP(1, 'RBF')
    m.fit(X, y, num_warmup=100, num_samples=100)
    obj = Thompson(jax.random.PRNGKey(0), m, X_new)
    assert_(isinstance(obj, jnp.ndarray))
    assert_equal(obj.squeeze().shape, (len(X_new),))


@pytest.mark.parametrize("acq", [EI, UCB, UE, POI, KG])
...
def test_acq_dkl(acq):
    ...
    m.fit(X, y, num_samples=5, num_warmup=5)
    obj = acq(m, X_new)
...
def test_UCB_beta():
    ...
    m = ExactGP(1, 'RBF')
    m.fit(X, y, num_warmup=100, num_samples=100)
    obj1 = UCB(m, X_new, beta=2)
    obj2 = UCB(m, X_new, beta=4)
    obj3 = UCB(m, X_new, beta=2)
    assert_(not onp.array_equal(obj1, obj2))
...
def test_EI_gp_penalty_inv_distance():
    ...
    m.fit(X, y, num_warmup=100, num_samples=100)
    obj1 = EI(m, X_new)
    obj2 = EI(m, X_new, penalty="inverse_distance", recent_points=recent_points)
...
def test_UCB_gp_penalty_inv_distance():
    ...
    m.fit(X, y, num_warmup=100, num_samples=100)
    obj1 = UCB(m, X_new)
    obj2 = UCB(m, X_new, penalty="inverse_distance", recent_points=recent_points)
    assert_(obj2[-1] < obj1[-1])
    assert_(obj2[-2] < obj1[-2])
...
def test_UE_gp_penalty_inv_distance():
    ...
    m.fit(X, y, num_warmup=100, num_samples=100)
    obj1 = UE(m, X_new)
    obj2 = UE(m, X_new, penalty="inverse_distance", recent_points=recent_points)
    assert_(obj2[-1] < obj1[-1])
    assert_(obj2[-2] < obj1[-2])
...
def test_acq_penalty_indices(acq, pen):
    ...
    m.fit(X, y, num_steps=50)
    obj1 = acq(m, X_new, grid_indices=indices,
...
def test_acq_error(acq_func):
    ...
    with pytest.raises(ValueError):
        acq_func(m, X_new, penalty='delta')

    # Test the ValueError for passing recent_points as non ndarray type
    with pytest.raises(ValueError):
        acq_func(m, X_new, penalty='delta', recent_points=[1, 2, 3])

    # Test the function with correct parameters
    try:
        output = acq_func(m, X, penalty='delta', recent_points=recent_points)
