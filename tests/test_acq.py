...
def test_acq_penalty():
    ...
    obj2 = UE(rng_keys[1], m, X_new, last_point=last_point)
    assert_(onp.count_nonzero(obj1 - obj2) > 0)


def test_acq_penalty_indices():
    rng_keys = get_keys()
    h = w = 5
    ...
    obj1 = UE(rng_keys[1], m, X_new, indices=indices, last_point=indices[7])
    obj2 = UE(rng_keys[1], m, X_new)
